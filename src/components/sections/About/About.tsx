...
const About = () => {
  ...
  const handleInViewChange = useCallback((inView: boolean) => {
    registerIntersection('about', inView);
  ...
  const features = [
    {
      ...
      description: 'Мы работаем с местными фермерами, чтобы получить самое свежее молоко для наших ремесленных сыров, поддерживая устойчивое сельское хозяйство.'
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      description: 'Мы используем проверенные временем техники, передаваемые из поколения в поколение, чтобы создавать аутентичные вкусы, которые невозможно повторить.'
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      description: 'Каждая головка сыра изготовлена с любовью и вниманием к деталям нашими мастерами-сыроделами, которые увлечены своим ремеслом.'
    },
  ];
    ...
    <section className={styles.about} id="about" ref={sectionRef}>
      <div className={styles.about__container}>
        <div className={styles.about__header}>
          ...
          >
            Откройте для себя наш путь <span>сыроделия</span>
          </motion.h2>
          ...
          >
            На протяжении более трех десятилетий мы создаем исключительные ремесленные сыры, которые чтят традиции, одновременно внедряя инновации. Наш семейный бизнес посвящен качеству, устойчивому развитию и искусству сыроделия.
          </motion.p>
        </div>
              ...
              {features.map((feature, index) => (
                ...
                >
                  ...
                  <div className={styles['about__feature-content']}>
                    <h3 className={styles['about__feature-title']}>{feature.title}</h3>
                    <p className={styles['about__feature-description']}>{feature.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
